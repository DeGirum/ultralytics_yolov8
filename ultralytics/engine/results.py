...
class Results(SimpleClass):
    ...
    def __init__(
        self, orig_img, path, names, boxes=None, masks=None, probs=None, keypoints=None, obb=None, regress=None, speed=None
    ) -> None:
        """
    ...
    def verbose(self):
        ...
        if regress := self.regress:
            return f"{', '.join(f'{self.names[j]} {regress.data[j]:.2f}' for j in regress.value)}, "
        if boxes := self.boxes:
