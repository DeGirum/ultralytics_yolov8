...
def verify_image_label(args):
    ...
    nm, nf, ne, nc, msg, segments, keypoints, mlb = 0, 0, 0, 0, "", [], None, None
    try:
        # Verify images
        im = Image.open(im_file)
        im.verify()  # PIL verify
...
def check_multi_label_det_dataset(dataset, autodownload=True):
    """
    Download, verify, and/or unzip a dataset if not found locally.
...
def check_multi_label_cls_dataset(dataset, autodownload=True):
    """
    Download, verify, and/or unzip a multi label classification dataset if not found locally.
